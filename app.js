...
function startLivePositionWatch() {
  if (livePositionWatchId !== null || !navigator.geolocation) return;
  livePositionWatchId = navigator.geolocation.watchPosition(
    (pos) => {
      ...
      if (guidanceWatchId === null) {
        smoothLocation = getCurrentUserLocation();
...
function setupEvents() {
  const unlockOrientation = () => {
    enableOrientationTracking().catch(() => {});
  };
  window.addEventListener('pointerdown', unlockOrientation, { once: true });
  window.addEventListener('touchstart', unlockOrientation, { once: true, passive: true });
